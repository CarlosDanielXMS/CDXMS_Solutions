...
for p in [CAP/'manifest.json',CAP/'contract.json',CAP/'config.default.json',CAP/'macrodroid'/'[CDXMS]_Artifact_Manager.ablock']:
    if not p.exists():errors.append(f'Arquivo ausente: {p}')
ab=load(CAP/'macrodroid'/'[CDXMS]_Artifact_Manager.ablock') if (CAP/'macrodroid'/'[CDXMS]_Artifact_Manager.ablock').exists() else {}
m=ab.get('macro',{})
...
if m.get('m_name')!='[CDXMS] Artifact Manager':errors.append('Nome interno divergente')
...
if errors:
 print('VALIDATION FAILED — artifact_manager');[print('-',e) for e in errors];raise SystemExit(1)
print('VALIDATION OK — artifact_manager v1.0.0')
